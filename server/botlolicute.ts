...
// Note: Some mineflayer plugins may not be available - gracefully handle missing plugins
const plugins = {
  armorManager: (() => { try { return require('mineflayer-armor-manager'); } catch { return null; } })(),
  autoEat: (() => { try { return require('mineflayer-auto-eat'); } catch { return null; } })(),
  collectBlock: (() => { try { return require('mineflayer-collectblock'); } catch { return null; } })(),
  pvp: (() => { try { return require('mineflayer-pvp'); } catch { return null; } })(),
  tool: (() => { try { return require('mineflayer-tool'); } catch { return null; } })()
};
...
// Removed droppedItemsLocation - no longer returning to pick up items after death
let originalGameMode = 'survival';
let isFlying = false;

let defaultMovements: any;
...
function setupBotEvents() {
  ...
  bot.on('spawn', () => {
    ...
    // Load smart mineflayer plugins for enhanced functionality
    try {
      if (plugins.armorManager) {
        bot.loadPlugin(plugins.armorManager);
        console.log('✅ Loaded armor manager plugin');
      }
      if (plugins.autoEat) {
        bot.loadPlugin(plugins.autoEat);
        ...
        console.log('✅ Loaded auto-eat plugin');
      }
      if (plugins.collectBlock) {
        bot.loadPlugin(plugins.collectBlock);
        console.log('✅ Loaded collect block plugin');
      }
      if (plugins.pvp) {
        bot.loadPlugin(plugins.pvp);
        console.log('✅ Loaded PvP plugin');
      }
      if (plugins.tool) {
        bot.loadPlugin(plugins.tool);
        console.log('✅ Loaded tool plugin');
      }
      
      console.log('🤖 Bot được trang bị plugins thông minh!');
    } catch (error) {
      console.log('⚠️ Một số plugins không khả dụng:', error);
    }

    setTimeout(() => {
      safeChat('Chào mừng đến với bot loli thông minh! 💕 Tôi đã được nâng cấp với AI và plugins xịn! (◕‿◕)♡✨');
    }, 2000);
  ...
  bot.on('death', () => {
    try {
      safeChat('Nooo! Tôi đã chết! (;´∀`) Respawning và tiếp tục nhiệm vụ!');
      // No longer returning to pick up items - start fresh after respawn
      setTimeout(() => {
        if (bot && isConnected) {
          safeChat('Tôi đã hồi sinh! Sẵn sàng cho nhiệm vụ mới! ✨(◕‿◕)♡');
          // Reset to idle mode after death
          stopCurrentActivity();
        }
      }, 3000);
    } catch (error) {
...
// ==================== PvP Hunt System ====================
function startPvPHunt(targetPlayer: any, requesterUsername: string) {
  if (!bot.pathfinder || !targetPlayer?.entity) {
    safeChat('Mou~ PvP system chưa sẵn sàng! (´;ω;) Thử lại sau nhé!');
    return;
  }

  console.log(`⚔️ Bắt đầu PvP hunt với ${targetPlayer.username}`);
  currentMode = 'pvp_hunt';
  let huntInterval: NodeJS.Timeout | null = null;
  let lastPvPChatTime = 0;
  let huntStartTime = Date.now();
  
  // Cute/arrogant PvP messages
  const pvpMessages = [
    `Ara ara~ ${targetPlayer.username}-kun! 💕 Bạn không thể trốn khỏi em đâu! ヽ(°〇°)ﾉ`,
    `${targetPlayer.username}! Em sẽ cho bạn biết sức mạnh thật sự! (ง •̀_•́)ง ✨`,
    `Kyaa~ ${targetPlayer.username}-chan chạy nhanh quá! Nhưng em sẽ bắt được thôi! ♪(´▽｀)`,
    `Mou~ ${targetPlayer.username}! Đừng chạy nữa mà! Em muốn chơi với bạn! (๑•̀ᄇ•́)و`,
    `Hihi~ ${targetPlayer.username}! Bạn có thể trốn được không nhỉ? (~￣▽￣)~`,
    `${targetPlayer.username}-kun! Em sẽ teleport đến bạn ngay! Wait for me~ ✨`
  ];

  huntInterval = setInterval(() => {
    if (!isConnected || !bot || currentMode !== 'pvp_hunt' || !targetPlayer?.entity) {
    ...
    try {
      ...
      botScreenData.targetPlayer = targetPlayer.username;
      botScreenData.status = `Đang PvP hunt ${targetPlayer.username} (${distance.toFixed(1)}m)`;
      
      // Random cute/arrogant chat messages every 10-15 seconds
      if (now - lastPvPChatTime > (10000 + Math.random() * 5000)) {
        const randomMessage = pvpMessages[Math.floor(Math.random() * pvpMessages.length)];
        safeChat(randomMessage);
        lastPvPChatTime = now;
      }

      // If target is more than 30 blocks away, use teleport
      if (distance > 30) {
        console.log(`🔄 Target too far (${distance.toFixed(1)}m), teleporting...`);
        safeChat(`Mou~ ${targetPlayer.username}! Xa quá rồi! Em sẽ teleport đến bạn! ✨`);
        bot.chat(`/tp ${targetPlayer.username}`);
        
        // Wait a bit after teleport before continuing attack
        setTimeout(() => {
          if (currentMode === 'pvp_hunt' && targetPlayer?.entity) {
            attackPlayer(targetPlayer);
          }
        }, 2000);
      }
      // If within attack range (1-5 blocks), attack directly
      else if (distance <= 5) {
        attackPlayer(targetPlayer);
      }
      // If within reasonable distance (6-30 blocks), chase with pathfinder
      else {
        console.log(`🏃 Chasing ${targetPlayer.username} (${distance.toFixed(1)}m away)`);
        const goal = new goals.GoalFollow(targetPlayer.entity, 1.5);
        bot.pathfinder.setGoal(goal, true);
      }
      
      updateBotScreen();
      
      // Auto-stop after 3 minutes to prevent infinite hunting
      if (now - huntStartTime > 180000) {
        safeChat(`Mou~ ${targetPlayer.username}! Em mệt rồi! PvP hunt kết thúc nhé! (￣▽￣)ノ`);
        stopCurrentActivity();
      }
      
    } catch (error) {
      console.log('🔧 Lỗi trong PvP hunt...');
      if (huntInterval) clearInterval(huntInterval);
      stopCurrentActivity();
    }
  }, 1000); // Check every second for responsive PvP
}

function attackPlayer(targetPlayer: any) {
  try {
    if (!bot || !targetPlayer?.entity) return;
    
    // Use PvP plugin if available for better combat
    if (plugins.pvp && bot.pvp) {
      bot.pvp.attack(targetPlayer.entity);
      console.log(`⚔️ Using PvP plugin to attack ${targetPlayer.username}`);
    } else {
      // Fallback to basic attack
      bot.attack(targetPlayer.entity);
      console.log(`⚔️ Basic attack on ${targetPlayer.username}`);
    }
    
    // Cute attack messages
    const attackMessages = [
      `Take this! ${targetPlayer.username}-kun! ⚔️✨`,
      `Ara ara~ ${targetPlayer.username}! Feel my power! (ง •̀_•́)ง`,
      `Kyaa~ ${targetPlayer.username}-chan! En garde! ⚡`,
      `Mou~ ${targetPlayer.username}! Em không mercy đâu! 💥`
    ];
    
    const randomAttackMsg = attackMessages[Math.floor(Math.random() * attackMessages.length)];
    
    // Don't spam attack messages - only every 5 seconds
    const now = Date.now();
    if (now - lastCombatChatTime > 5000) {
      safeChat(randomAttackMsg);
      lastCombatChatTime = now;
    }
    
  } catch (error) {
    console.log('🔧 Lỗi attack player...');
  }
}
...
async function processUserMessage(username: string, message: string) {
  try {
    ...
    if (lowerMessage.includes('auto armor') || lowerMessage.includes('mặc giáp')) {
      console.log('➡️ Lệnh: Auto armor');
      if (bot.armorManager) {
        bot.armorManager.equipAll();
        safeChat(`Hai ${username}-chan! Tôi đã tự động mặc giáp tốt nhất! ⚔️✨`);
      } else {
        safeChat(`Gomen ${username}-chan! Plugin armor chưa sẵn sàng! (´;ω;)`);
      }
      return;
    }
    
    if (lowerMessage.includes('auto tool') || lowerMessage.includes('dụng cụ')) {
      console.log('➡️ Lệnh: Auto tool');
      const toolMatch = lowerMessage.match(/auto tool (\w+)/) || lowerMessage.match(/dụng cụ (\w+)/);
      if (toolMatch && bot.tool) {
        const toolType = toolMatch[1];
        try {
          await bot.tool.equipForBlock(bot.blockAt(bot.entity.position.offset(0, -1, 0)));
          safeChat(`Đã trang bị tool phù hợp cho ${toolType}! ⛏️`);
        } catch (error) {
          safeChat(`Không tìm thấy tool phù hợp cho ${toolType}! (´;ω;)`);
        }
      } else {
        safeChat(`${username}-chan! Hãy nói rõ loại tool! VD: "auto tool pickaxe" ⛏️`);
      }
      return;
    }
    ...
    if (lowerMessage.includes('heal me') || lowerMessage.includes('chữa em')) {
      console.log('➡️ Lệnh: Heal');
      if (bot.autoEat) {
        bot.autoEat.eat();
        safeChat(`${username}-chan! Tôi sẽ ăn để hồi máu! 🍎✨`);
      } else {
        safeChat(`Gomen ${username}-chan! Plugin auto-eat chưa sẵn sàng! (´;ω;)`);
      }
      return;
    }
    
    if (lowerMessage.startsWith('pvp ')) {
      console.log('➡️ Lệnh: PvP Target');
      const targetMatch = message.match(/pvp\s+(\w+)/i);
      if (targetMatch) {
        const targetName = targetMatch[1];
        const foundPlayer = bot?.players?.[targetName];
        
        if (foundPlayer && foundPlayer.entity) {
          stopCurrentActivity();
          currentMode = 'pvp_hunt';
          // Set the global targetPlayer variable to the found player
          targetPlayer = foundPlayer;
          currentCommand = `pvp ${targetName}`;
          lastActivityTime = Date.now();
          
          safeChat(`Ara ara~ ${targetName}-kun! 💕 Em sẽ đuổi theo anh đó! Hehe~ Không thể trốn được đâu! (๑•̀ᄇ•́)و ✨`);
          startPvPHunt(foundPlayer, username);
        } else {
          safeChat(`Mou~ ${username}-chan! Em không tìm thấy ${targetName} đâu! (´;ω;) Họ có online không vậy?`);
        }
      } else {
        safeChat(`${username}-chan! Hãy nói rõ tên người cần đánh! VD: "pvp PlayerName" ⚔️`);
      }
      return;
    }
    
    if (lowerMessage.includes('hãy nói ')) {
      console.log('➡️ Lệnh: Repeat message');
      const sayMatch = message.match(/hãy nói\s+(.+)/i);
      if (sayMatch) {
        const messageToSay = sayMatch[1];
        safeChat(messageToSay);
      }
      return;
  ...
  const selfDefenseInterval = setInterval(() => {
    ...
    try {
      ...
      if (nearbyMobs.length > 0) {
        ...
        bot.attack(target);
        botScreenData.status = `Đang tấn công ${(target as any).displayName || 'mob'}!`;
      } else {
...
async function startAutoFishing(username: string) {
  ...
  let waterBlock = null;
  for(let i = 0; i < 20; i++) {
    waterBlock = bot.findBlock({ matching: (block: any) => block.name === 'water' });
    if(waterBlock) break;
  ...
  bot.pathfinder.on('goal_reached', async () => {
    ...
    const fishingInterval = setInterval(() => {
      ...
      if(threats.length > 0) {
        ...
        stopCurrentActivity();
        currentMode = 'protecting';
        bot.attack((threats[0] as any).entity);
      } else {
        bot.fish.castFish();
        bot.once('caught_fish', (item: any) => {
...
function checkInventoryForItem(itemName: string) {
  try {
    ...
    for (const item of bot.inventory.items()) {
      ...
      const matchesSearch = searchTerms.some((term: any) => 
  ...
  try {
    ...
    for (const pickaxe of pickaxes) {
      ...
      if (item) {
        ...
        botScreenData.equipment.weapon = pickaxe as any;
        return pickaxe;
      }
    }
...
// returnToDroppedItems function removed - no longer needed
